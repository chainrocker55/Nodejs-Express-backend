...
const url = 'mongodb://tata:tata555@ds157223.mlab.com:57223/mobile'
...
app.get('/loginDisplay/:id', (req, res) => {
    ...
    mongoClient.connect(url, (err, client) => { // <= callback
        ...
        db.collection('user').findOne({'_id': new ObjectId(req.params.id)}, (err, result) =>{ // <= callback
            if (err) throw err;
...
app.get('/time/:id', (req, res) => {
    ...
    mongoClient.connect(url, (err, client) => { // <= callback
        ...
        const db = client.db(dbName)
        // findOne คือการค้นหาเพียงหนึ่ง collection โดยใช้เงื่อนไขการค้นหาได้ (ใน { })
        db.collection('time').find({'_id': new ObjectId(req.params.id)}, (err, result) =>{ // <= callback
            if (err) throw err;
            console.log(result);
            // res.json ทำการแปลง object เป็น string json เพื่อส่งไปยังหน้าเว็บ
            res.json(result);
...
app.post('/register', (req, res) => {
    mongoClient.connect(url, (err, client) => {
        const db = client.db(dbName)
        db.collection('user').findOne({ email: req.body.email }, (err, result) => { // <= callback
            if (err) throw err
    ...
    mongoClient.connect(url).then(function (client) {
        ...
        db.collection('user').findOne({ email: req.body.email }, (err, result) => { // <= callback
            ...
            if (result === null) {
                ...
                db.collection("user").findOne({'email': req.body.email}).then((data) => {
                    if(data != null) {
                        const newDate = new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok'})
                        console.log(data);
                        data.times = [...data.times, newDate]
                        console.log(data);
                        var newvalues = {$set: {email: data.email,password: data.password,times: data.times,facebookLogin: data.facebookLogin}};
                        // db.collection("user").updateOne({'email': data.email },data).then((err ,res) => {
                        db.collection("user").updateOne({email: data.email},newvalues, (err, res) => {
                            if (err) throw err;
                        })
                        res.json({
                            status: true,
...
app.post('/login', (req, res) => {
    ...
    mongoClient.connect(url).then(function (client) {
        const db = client.db(dbName)
        //console.log(req.body.email)
        db.collection("user").findOne({'email': req.body.email, 'password': req.body.password }).then((data) => {
            if(data != null) {
                const newDate = new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok'})
                console.log(data);
                data.times = [...data.times, newDate]
